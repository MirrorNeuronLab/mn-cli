...
def test_custom_blueprint_storage_dir_is_sanitized_and_stable(monkeypatch, tmp_path):
    ...
    first = repo.custom_blueprint_storage_dir(
        "https://example.test/acme/customer blueprints.git"
    )
    second = repo.custom_blueprint_storage_dir(
        "https://example.test/acme/customer blueprints.git/"
    )
...
def test_load_blueprint_index_rejects_entries_without_paths_when_required(tmp_path):
    ...
    with pytest.raises(
        repo.BlueprintIndexError, match="ordered list of blueprint folder paths"
    ):
