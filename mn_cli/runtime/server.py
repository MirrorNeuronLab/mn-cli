...
from mn_sdk.runtime_config import (
    DEFAULT_API_PORT as SDK_DEFAULT_API_PORT,
    DEFAULT_CORE_HOST as SDK_DEFAULT_CORE_HOST,
...
DEFAULT_LLM_MODEL_RUNNER_MODEL = runtime_model_selection_policy("default")[0]
DEFAULT_CONTEXT_MODEL_RUNNER_MODEL = DEFAULT_CONTEXT_ENGINE_MODEL
DEFAULT_MODEL_RUNNER_PROXY_PORT = SDK_DEFAULT_MODEL_RUNNER_PROXY_PORT
...
DEFAULT_HOST = SDK_DEFAULT_CORE_HOST
DEFAULT_GRPC_PORT = SDK_DEFAULT_GRPC_PORT
DEFAULT_API_PORT = SDK_DEFAULT_API_PORT
...
def _ensure_litellm_gateway_host_config() -> None:
    try:
        ...
        if not LITELLM_GATEWAY_CONFIG_FILE.exists():
            LITELLM_GATEWAY_CONFIG_FILE.write_text(
                '{"model_list":[]}\n', encoding="utf-8"
            )
...
DEFAULT_WEB_UI_PORT = SDK_DEFAULT_WEB_UI_PORT
...
def _openshell_gateway_endpoint(env: Optional[dict[str, str]] = None) -> str:
    ...
    if not gateway_name:
        try:
            gateway_name = (
                (_openshell_config_dir() / "active_gateway")
    ...
    if gateway_name:
        try:
            ...
            metadata = {}
        endpoint = (
            metadata.get("gateway_endpoint") if isinstance(metadata, dict) else None
        )
...
def _detect_host_gpu_count() -> int:
    ...
    if system_name == "Darwin":
        ...
        if result.returncode == 0:
            return sum(
                1 for line in result.stdout.splitlines() if "Chipset Model:" in line
            )
        return 0
...
def _version_after(text: str, label: str) -> str:
    match = re.search(
        rf"{re.escape(label)}\s*[:=\- ]*\s*([0-9]+(?:\.[0-9]+)*)", text, re.I
    )
    ...
    if system_name == "Linux":
        ...
        for line in lspci.splitlines():
            ...
            if (
                ...
                or "radeon" in normalized
            ):
                rocm = (
                    os.getenv("ROCM_VERSION", "").strip()
                    or os.getenv("HIP_VERSION", "").strip()
                )
                profile = {
...
def _ensure_node_advertisement_settings(env: dict[str, str]) -> dict[str, str]:
    ...
    if not str(
        adjusted.get("MN_NODE_DISPLAY_NAME") or os.getenv("MN_NODE_DISPLAY_NAME", "")
    ).strip():
...
def _compose_runtime_env(env: dict[str, str], ip: Optional[str]) -> dict[str, str]:
    ...
    if ip:
        ...
        local_host = (
            str(compose_env.get("MN_NETWORK_ADVERTISE_HOST") or "").strip()
            or _detect_lan_ip()
        )
        docker_mode_value = (
            str(compose_env.get("MN_DOCKER_NETWORK_MODE") or "").strip().lower()
        )
        node_alias = str(compose_env.get("MN_NODE_ALIAS") or "").strip()
        docker_identity = (
            _docker_network_uses_internal_identity(docker_mode_value) and node_alias
        )
        local_node_name = (
            _docker_node_name(node_alias)
            if docker_identity
            else _network_node_name(local_host)
        )
        ...
        if _generated_node_setting_should_update(
            "MN_NODE_NAME", compose_env.get("MN_NODE_NAME"), local_node_name
        ):
            compose_env["MN_NODE_NAME"] = local_node_name
        if (
            (
                not docker_identity
                and _generated_cluster_setting_should_update(
                    compose_env.get("MN_CLUSTER_NODES"), seed_node_name
                )
            )
            or (
                docker_identity
                and _cluster_nodes_unset(compose_env.get("MN_CLUSTER_NODES"))
            )
            or compose_env.get("MN_CLUSTER_NODES") == ip
        ):
            compose_env["MN_CLUSTER_NODES"] = seed_node_name
        compose_env["MN_REDIS_URL"] = (
            compose_env.get("MN_REDIS_URL") or f"redis://{ip}:6379/0"
        )
...
def _container_publishes_port(
    container_name: str, target_port: int, published_port: int
) -> bool:
    try:
        result = subprocess.run(
...
def _port_available_or_owned(
    host: str, port: int, owner_container: str, target_port: int
) -> bool:
    return _host_port_available(host, port) or _container_publishes_port(
        owner_container, target_port, port
    )


def _find_available_published_port(
    host: str, preferred: int, owner_container: str, target_port: int
) -> int:
    ...
    *,
    ...
    if explicit:
        if requested_port is None:
            print_error(
                console, "MN_REDIS_PORT must be a TCP port between 1 and 65535."
            )
            raise typer.Exit(1)
        if not _port_available_or_owned(
            bind_host, requested_port, owner_container, REDIS_CONTAINER_PORT
        ):
    ...
    if (
        current_port
        and REDIS_DYNAMIC_PORT_START <= current_port <= REDIS_DYNAMIC_PORT_END
    ):
        return current_port
    ...
    if selected == 0:
        print_error(
            console,
            f"No Redis port is available in {REDIS_DYNAMIC_PORT_START}-{REDIS_DYNAMIC_PORT_END}.",
        )
        raise typer.Exit(1)
    return selected
...
def _avoid_local_compose_port_conflicts(env: dict[str, str]) -> dict[str, str]:
    ...
    adjusted = dict(env)
    allocation_mode = (
        str(adjusted.get("MN_BLUEPRINT_WEB_UI_PORT_ALLOCATION_MODE") or "")
        .strip()
        .lower()
    )
    ...
    bind_host = str(
        adjusted.get("MN_BLUEPRINT_WEB_UI_BIND_HOST")
...
def _resolve_redis_password(env: Optional[dict[str, str]] = None) -> str:
    ...
    admin_token = (
        str((env or {}).get(GRPC_ADMIN_TOKEN_ENV) or "").strip()
        or _resolve_grpc_admin_token()
    )
...
def _ensure_runtime_api_token(
    env: dict[str, str], *, persist_compose: bool = False
) -> dict[str, str]:
    resolved = dict(env)
    ...
    if persist_compose and runtime_compose_available():
        _write_env_file_values(
            RUNTIME_COMPOSE_ENV, {"MN_API_TOKEN": resolved["MN_API_TOKEN"]}
        )
    return resolved


def _ensure_runtime_grpc_tokens(
    env: dict[str, str], *, persist_compose: bool = False
) -> dict[str, str]:
    resolved = dict(env)
    fixed_tokens = {
...
def _shared_storage_env_values(host_root: str, runtime_root: str) -> dict[str, str]:
    runtime_root = (
        str(runtime_root or DEFAULT_RUNTIME_SHARED_STORAGE_ROOT).strip()
...
def _shared_storage_env_from_handshake(handshake: Optional[dict]) -> dict[str, str]:
    ...
    if not expanded_host_root.exists():
        print_warning(
            console, f"Remote shared-storage root is not a local path: {host_root}"
        )
        print_warning(
            console,
...
def _syncthing_enabled(env: dict[str, str]) -> bool:
    value = (
        str(
            env.get("MN_SYNCTHING_ENABLED")
            or os.getenv("MN_SYNCTHING_ENABLED")
            or "auto"
        )
        .strip()
        .lower()
...
def _syncthing_required(env: dict[str, str]) -> bool:
    return _truthy_env(
        env.get("MN_SYNCTHING_REQUIRED") or os.getenv("MN_SYNCTHING_REQUIRED")
    )
...
def _syncthing_image(env: dict[str, str]) -> str:
    return str(
        env.get("MN_SYNCTHING_IMAGE")
        or os.getenv("MN_SYNCTHING_IMAGE")
        or DEFAULT_SYNCTHING_IMAGE
    ).strip()


def _syncthing_api_key(env: dict[str, str]) -> str:
    configured = str(
        env.get("MN_SYNCTHING_API_KEY") or os.getenv("MN_SYNCTHING_API_KEY") or ""
    ).strip()
    if configured:
        return configured
    try:
...
def _syncthing_folder_id(env: dict[str, str]) -> str:
    return str(
        env.get("MN_SYNCTHING_FOLDER_ID")
        or os.getenv("MN_SYNCTHING_FOLDER_ID")
        or SYNCTHING_FOLDER_ID
    ).strip()


def _syncthing_folder_path(env: dict[str, str]) -> str:
    return str(
        env.get("MN_SYNCTHING_FOLDER_PATH")
        or os.getenv("MN_SYNCTHING_FOLDER_PATH")
        or SYNCTHING_FOLDER_PATH
    ).strip()
...
def _syncthing_rescan_interval_seconds(value: object = None) -> int:
    ...
    try:
        interval = (
            ...
            else DEFAULT_SYNCTHING_RESCAN_INTERVAL_SECONDS
        )
    except ValueError:
...
def _start_compose_syncthing_service(
    env: dict[str, str], updates: dict[str, str]
) -> None:
...
def _resolve_syncthing_port(
    ...
    *,
    ...
    for candidate in range(default + 1, default + 50):
        if _port_available_or_owned(
            bind_host, candidate, SYNCTHING_CONTAINER, target_port
        ):
            return candidate
...
def _wait_for_syncthing_api(
    host: str, port: int, api_key: str, *, timeout_seconds: float = 20.0
) -> None:
    ...
    while time.time() < deadline:
        try:
            _syncthing_request(
                host, port, api_key, "GET", "/rest/system/ping", timeout=1.0
            )
            return
            ...
            time.sleep(0.5)
    raise RuntimeError(
        f"Syncthing API at {host}:{port} did not become ready: {last_error}"
    )
...
def _ensure_syncthing_folder(
    info: dict[str, Any], peers: tuple[dict[str, Any], ...] = ()
) -> None:
    ...
    for peer in peers:
        ...
        existing = next(
            (item for item in devices if item.get("deviceID") == peer_id), None
        )
        addresses = (
            [f"tcp://{peer.get('host')}:{peer.get('sync_port')}"]
            if peer.get("host") and peer.get("sync_port")
            else ["dynamic"]
        )
        if existing is None:
            devices.append(
    ...
    if folder is None:
        ...
        existing_ids = {
            str(item.get("deviceID") or "") for item in folder.get("devices", [])
        }
    ...
    if config != original_config:
        ...
        try:
            _syncthing_request(
                host, port, api_key, "POST", "/rest/system/restart", timeout=2.0
            )
        except Exception:
            pass
        try:
...
def _ensure_syncthing_for_runtime(
    env: dict[str, str], *, advertised_host: str
) -> dict[str, str]:
    ...
    try:
        ...
        _syncthing_warn_or_fail(
            f"could not create Syncthing shared-storage directories: {exc}", env
        )
        return env
    ...
    if _ensure_compose_syncthing_service_definition():
        try:
            ...
            _syncthing_warn_or_fail(
                f"could not start Compose Syncthing sidecar: {exc}", env
            )
            return env
    else:
        recreate = not _docker_container_running(SYNCTHING_CONTAINER)
        if not recreate:
            current_root = _docker_container_env_value(
                SYNCTHING_CONTAINER, "MN_HOST_SHARED_STORAGE_ROOT"
            )
            current_api_key = _docker_container_env_value(
                SYNCTHING_CONTAINER, "MN_SYNCTHING_API_KEY"
            )
            lan_only_marker = _docker_container_env_value(
        ...
        if recreate:
            ...
            subprocess.run(
                ["docker", "start", SYNCTHING_CONTAINER],
                check=False,
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL,
            )
    ...
    try:
        ...
        _syncthing_warn_or_fail(
            f"Syncthing shared-storage sidecar is not reachable: {exc}", env
        )
        return env

    device_id = str(
        status.get("myID") or env.get("MN_SYNCTHING_DEVICE_ID") or ""
    ).strip()
    ...
    print_info(
        console,
        f"Syncthing shared storage is ready at {advertised_host}:{sync_port} ({host_root}).",
    )
    return env
...
def _syncthing_node_info(env: dict[str, str], advertised_host: str) -> dict[str, Any]:
    ...
    return {
        ...
        "gui_port": _parse_configured_port(env.get("MN_SYNCTHING_GUI_PORT"))
        or DEFAULT_SYNCTHING_GUI_PORT,
        "sync_port": _parse_configured_port(env.get("MN_SYNCTHING_SYNC_PORT"))
        or DEFAULT_SYNCTHING_SYNC_PORT,
        "folder_id": _syncthing_folder_id(env),
        "folder_path": _syncthing_folder_path(env),
...
def _reconcile_syncthing_federated_peers(
    ...
    *,
    ...
    try:
        ...
        logger.info(
            "Could not read federation peers for Syncthing reconciliation: %s", exc
        )
        return {"discovered": 0, "connected": 0}
...
def _compose_shared_storage_env_changed(updates: dict[str, str]) -> bool:
    ...
    return any(
        str(current.get(key) or "").strip() != str(value or "").strip()
...
def _recreate_compose_core_for_shared_storage(env: dict[str, str]) -> None:
    if not runtime_compose_available():
        return
    print_info(
        console, "Recreating MirrorNeuron core so shared-storage changes are visible…"
    )
    try:
        subprocess.run(
...
def _resolve_network_token(force_new: bool = False) -> str:
    ...
    compose_token = (
        _read_env_file(RUNTIME_COMPOSE_ENV).get("MN_NETWORK_JOIN_TOKEN", "").strip()
    )
    if compose_token:
...
def _docker_network_mode(mode: Optional[str] = None, *, default: str = "bridge") -> str:
    raw = (
        (mode or os.getenv("MN_DOCKER_NETWORK_MODE", "").strip() or default)
        .strip()
        .lower()
    )
    if raw in {"", "bridge"}:
...
def _inspect_docker_network(name: str) -> Optional[dict[str, object]]:
    ...
    try:
        ...
        print_error(
            console, f"Could not parse Docker network inspect output for {name}."
        )
        raise typer.Exit(1)
...
def _ensure_docker_network(mode: str, name: str) -> None:
    ...
    if mode == "bridge":
        ...
        if str(inspected.get("Driver") or "") != "bridge":
            print_error(
                console, f"Docker network {name} exists but is not a bridge network."
            )
            raise typer.Exit(1)
        return
    ...
    if str(inspected.get("Driver") or "") != "overlay":
        print_error(
            console, f"Docker network {name} exists but is not an overlay network."
        )
        raise typer.Exit(1)
...
def _generated_node_setting_should_update(
    key: str, value: object, desired: str
) -> bool:
...
def _native_sdk_grpc_node_info(
    ...
    bind_host = str(
        values.get("MN_NATIVE_SDK_GRPC_HOST") or DEFAULT_NATIVE_SDK_GRPC_HOST
    ).strip()
...
def _handshake_with_main_node(
    ...
    *,
    ...
    advertised_node_name = local_node_name or (
        _network_node_name(local_host) if local_host else ""
    )
    local_node_info = (
...
def _running_core_has_stale_grpc_tokens(
    container_name: str = "mirror-neuron-core",
) -> bool:
    expected = {
        "MN_GRPC_AUTH_TOKEN": FIXED_GRPC_AUTH_TOKEN,
        GRPC_ADMIN_TOKEN_ENV: FIXED_GRPC_ADMIN_TOKEN,
    ...
    return any(
        _docker_container_env_value(container_name, key) != value
...
def _ensure_network_docker_network(
    mode: str = "bridge", name: Optional[str] = None
) -> None:
    _ensure_docker_network(mode, name or NETWORK_DOCKER_NETWORK)


def _network_redis_url(
    token: str, redis_host: str, redis_port: int, password: Optional[str] = None
) -> str:
...
def _redis_ha_mode(env: dict[str, str], *, cluster: bool = False) -> str:
    configured = (
        str(os.getenv("MN_REDIS_HA_MODE") or env.get("MN_REDIS_HA_MODE") or "")
        .strip()
        .lower()
    )
    if configured:
...
def _ensure_redis_ha_settings(
    ...
    *,
    ...
    redis_password = str(
        adjusted.get("MN_REDIS_PASSWORD") or ""
    ).strip() or _resolve_redis_password(adjusted)
    adjusted["MN_REDIS_PASSWORD"] = redis_password
    adjusted.setdefault(
        "MN_REDIS_SENTINELS", _redis_sentinels(adjusted, advertised_host)
    )
    ...
    adjusted.setdefault(
        "MN_REDIS_SENTINEL_PASSWORD",
        os.getenv("MN_REDIS_SENTINEL_PASSWORD", "").strip() or redis_password,
    )
    adjusted.setdefault(
        "MN_REDIS_WAIT_REPLICAS", os.getenv("MN_REDIS_WAIT_REPLICAS", "").strip() or "0"
    )
    adjusted.setdefault(
        "MN_REDIS_WAIT_TIMEOUT_MS",
        os.getenv("MN_REDIS_WAIT_TIMEOUT_MS", "").strip() or "1000",
    )
...
def _network_core_env(
    *,
    ...
    env.update(
        {
            ...
            or DEFAULT_BLUEPRINT_WEB_UI_BIND_HOST,
            "MN_BLUEPRINT_WEB_UI_PUBLIC_HOST": env.get(
            ...
            "MN_NATIVE_SDK_GRPC_ADVERTISE_HOST": env.get(
    ...
    env["MN_NATIVE_SDK_GRPC_TARGET"] = env.get(
        "MN_NATIVE_SDK_GRPC_TARGET"
    ) or _network_native_sdk_grpc_target(
        env,
...
def _network_native_sdk_grpc_target(
    env: dict[str, str], *, docker_network_mode: str
) -> str:
...
def _persist_worker_compose_foundation_env(env: dict[str, str]) -> None:
    ...
    updates = {
        ...
        "MN_NATIVE_SDK_GRPC_HOST": env.get(
            "MN_NATIVE_SDK_GRPC_HOST", DEFAULT_NATIVE_SDK_GRPC_HOST
        ),
        "MN_NATIVE_SDK_GRPC_PORT": env.get(
            "MN_NATIVE_SDK_GRPC_PORT", DEFAULT_NATIVE_SDK_GRPC_PORT
        ),
        "MN_NATIVE_SDK_GRPC_ADVERTISE_HOST": env.get(
            "MN_NATIVE_SDK_GRPC_ADVERTISE_HOST", ""
        ),
...
def _start_worker_compose_foundation_services(env: dict[str, str]) -> None:
    ...
    available_services = [
        service for service in services if f"{service}:" in compose_text
    ]
...
def _network_shared_storage_roots(env: dict[str, str]) -> tuple[str, str, str]:
    ...
    bundle_cache_dir = str(
        env.get("MN_BUNDLE_CACHE_DIR") or f"{runtime_root.rstrip('/')}/bundle_cache"
    ).strip()
...
def _docker_env_args(env: dict[str, str]) -> list[str]:
    ...
    for key in sorted(env):
        if key.startswith("MN_") or key in {
            "ERL_AFLAGS",
            "ERL_EPMD_ADDRESS",
            "ERL_EPMD_PORT",
        }:
...
def _docker_worker_bind_args() -> list[str]:
    ...
    if socket_path is not None:
        ...
        docker_cli = (
            shutil.which("docker") if sys.platform.startswith("linux") else None
        )
        if docker_cli:
...
def _start_network_redis(
    ...
    *,
    ...
    subprocess.run(
        ["docker", "rm", "-f", NETWORK_REDIS_CONTAINER],
        stderr=subprocess.DEVNULL,
        stdout=subprocess.DEVNULL,
    )
    ...
    cmd = [
        ...
        *_docker_network_run_args(
            docker_network_mode, docker_network_name, redis_alias
        ),
        "-e",
    ...
    if publish_host_port:
        ...
        cmd[volume_index:volume_index] = [
            "-p",
            f"{publish_host}:{redis_port or REDIS_CONTAINER_PORT}:6379",
        ]
...
def _start_redis_sentinel(
    *,
    ...
    quorum = str(
        env.get("MN_REDIS_SENTINEL_QUORUM")
        or os.getenv("MN_REDIS_SENTINEL_QUORUM")
        or "1"
    )
    ...
    cmd = [
        ...
        container_name,
        *_docker_network_run_args(
            docker_network_mode, docker_network_name, sentinel_alias
        ),
        "-v",
        f"{data_dir}:/data",
        _network_redis_image(),
...
def _start_compose_sentinel(advertised_host: str, env: dict[str, str]) -> None:
    redis_port = (
        _parse_configured_port(env.get("MN_REDIS_PORT")) or REDIS_DYNAMIC_PORT_START
    )
    _start_redis_sentinel(
...
def _configure_worker_sentinel_primary(
    ...
    *,
    ...
    redis_ha = node_info.get("redis_ha") if isinstance(node_info, dict) else None
    if (
        not isinstance(redis_ha, dict)
        or str(redis_ha.get("mode") or "").strip().lower() != "sentinel"
    ):
        return None

    sentinel_port = (
        _parse_configured_port(redis_ha.get("sentinel_port"))
        or DEFAULT_REDIS_SENTINEL_PORT
    )
    sentinel_password = str(
        redis_ha.get("sentinel_password") or ""
    ).strip() or _redis_password_from_url(str(worker_handshake.get("redis_url") or ""))
    master_name = (
        str(redis_ha.get("sentinel_master") or "mirror-neuron").strip()
        or "mirror-neuron"
    )
...
def _start_network_core(
    ...
    *,
    ...
    subprocess.run(
        ["docker", "rm", "-f", NETWORK_CORE_CONTAINER],
        stderr=subprocess.DEVNULL,
        stdout=subprocess.DEVNULL,
    )
    publish_host = _network_publish_host(host)
    ...
    if publish_cluster_ports:
        epmd_port = _parse_configured_port(env.get("MN_EPMD_PORT")) or int(
            DEFAULT_EPMD_PORT
        )
        port_args.extend(
            [
...
def _running_network_host(
    host: Optional[str], container_names: tuple[str, ...] = ()
) -> str:
    ...
    for container_name in container_names:
        advertised_host = _docker_container_env_value(
            container_name, "MN_NETWORK_ADVERTISE_HOST"
        )
        if advertised_host:
            return advertised_host

    advertised_host = (
        _read_env_file(RUNTIME_COMPOSE_ENV).get("MN_NETWORK_ADVERTISE_HOST", "").strip()
    )
    if advertised_host:
        return advertised_host
...
def _print_network_seed_ready(
    ...
    *,
    ...
    network_args = _docker_network_command_args(
        docker_network_mode, docker_network_name
    )
    details = [
        ("Host", host),
...
def _print_runtime_join_ready(
    ...
    *,
    ...
    advertised_host = str(
        env.get("MN_NETWORK_ADVERTISE_HOST") or ""
    ).strip() or _running_network_host(host, container_names)
...
def _return_running_network_seed(
    ...
    if not token:
        print_error(
            console, "MirrorNeuron is already running, but no network token was found."
        )
    ...
    if running_tokens:
        _ensure_runtime_grpc_tokens(
            running_tokens, persist_compose=runtime_compose_available()
        )

    advertised_host = _running_network_host(host, container_names)
    node_name = (
        _docker_container_env_value(container_names[0], "MN_NODE_NAME")
        if container_names
        else None
    )
    docker_mode = (
        _docker_container_env_value(container_names[0], "MN_DOCKER_NETWORK_MODE")
        if container_names
        else None
    )
    docker_network = (
        _docker_container_env_value(container_names[0], "MN_DOCKER_NETWORK_NAME")
        if container_names
        else None
    )
...
def _stop_local_runtime_for_worker() -> None:
    ...
    if runtime_compose_available():
        ...
        try:
            ...
            logger.debug(
                "Failed to prune DockerWorker Compose services during local runtime stop",
                exc_info=True,
            )
        subprocess.run(
            ["docker", "rm", "-f", COMPOSE_SENTINEL_CONTAINER],
            stderr=subprocess.DEVNULL,
            stdout=subprocess.DEVNULL,
        )
        subprocess.run(
        ...
        subprocess.run(
            ["docker", "rm", "-f", LOCAL_CORE_CONTAINER],
            stderr=subprocess.DEVNULL,
            stdout=subprocess.DEVNULL,
        )
        subprocess.run(
    ...
    for pid_file, _name in [
        ...
        *native_sdk_grpc_pid_files(),
        ...
        try:
            ...
            logger.debug(
                "Failed to remove runtime pid file %s during worker start",
                pid_file,
                exc_info=True,
            )
...
def _start_network_seed(
    ...
    redis_url = external_redis_url or (
        ...
        if use_internal_identity
        else _network_redis_url(
            token, host, selected_redis_port or REDIS_CONTAINER_PORT, redis_password
        )
    )
    redis_public_host, redis_public_port_value = (
...
def _running_core_self_node(client: Any) -> dict[str, Any]:
    ...
    node = next(
        (
            item
            for item in nodes
            if item.get("self") is True or item.get("self?") is True
        ),
...
def _reconcile_cluster_models_after_membership_change() -> dict[str, Any] | None:
    try:
        ...
        print_warning(
            console, f"Cluster joined, but model route reconciliation failed: {exc}"
        )
        return None
    ...
    *,
    ...
    if _docker_network_uses_internal_identity(docker_network_mode):
        ...
        env = _ensure_compose_cluster_port_settings(
            env, token=primary_token, advertised_host=local_host
        )
...
def _wait_for_local_cluster_grpc(
    ...
    *,
    ...
    print_error(
        console,
        "Local MirrorNeuron core did not become ready after enabling cluster mode.",
    )
...
def compose_web_ui_enabled() -> bool:
    ...
    profiles = {
        profile.strip()
        for profile in _read_env_file(RUNTIME_COMPOSE_ENV)
...
def record_runtime_model_install(entry: dict[str, Any]) -> Optional[Path]:
    ...
    existing = (
        _read_env_file(RUNTIME_COMPOSE_ENV) if RUNTIME_COMPOSE_ENV.exists() else {}
    )
    ...
    try:
        ...
        logger.debug(
            "Could not record runtime model service advertisement", exc_info=True
        )
    if _is_default_llm_model(entry):
        updates["MN_LLM_MODEL_RUNNER_MODEL"] = model_id
    elif not str(existing.get("MN_LLM_MODEL_RUNNER_MODEL") or "").strip():
...
def _install_context_engine_model(model: str) -> str:
    ...
    if run_result.returncode != 0:
        ...
        if "already" not in output.lower():
            raise RuntimeError(
                f"Failed to start context engine model {model}: {output}"
            )
    return "installed"
...
def _anonymous_public_gar_docker_env(
    env: dict[str, str], image: str
    ...
    if source_config.exists():
        try:
            ...
            logger.debug(
                "Could not read Docker config for public GAR pull; using anonymous config",
                exc_info=True,
            )
...
def _expose_docker_cli_plugins(
    source_config_dir: Path, target_config_dir: Path
) -> None:
    ...
    try:
        ...
        try:
            ...
            logger.debug(
                "Could not expose Docker CLI plugins for anonymous GAR pull",
                exc_info=True,
            )
...
def _expose_docker_contexts(source_config_dir: Path, target_config_dir: Path) -> None:
    ...
    try:
        ...
        try:
            ...
            logger.debug(
                "Could not expose Docker contexts for anonymous GAR pull", exc_info=True
            )


def _is_public_gar_image(image: str) -> bool:
    text = str(image or "").strip()
    return PUBLIC_GAR_PROJECT_PATH in text and _docker_image_registry_host(
        text
    ).endswith(".pkg.dev")
...
def _docker_config_dir(env: dict[str, str]) -> Path:
    configured = str(
        env.get("DOCKER_CONFIG") or os.environ.get("DOCKER_CONFIG") or ""
    ).strip()
    return Path(configured).expanduser() if configured else Path.home() / ".docker"


def _docker_config_without_public_gar_credentials(
    config: dict[str, Any], registry_host: str
) -> dict[str, Any]:
...
def _docker_registry_matches_public_gar(registry: str, registry_host: str) -> bool:
    ...
    normalized = (
        normalized.removeprefix("https://").removeprefix("http://").split("/", 1)[0]
    )
...
def _context_engine_git_url(env: dict[str, str]) -> str:
    explicit = str(
        os.getenv("MN_MEMBRANE_GIT_URL") or env.get("MN_MEMBRANE_GIT_URL") or ""
    ).strip()
    if explicit:
        return explicit
    repo = str(
        os.getenv("MN_MEMBRANE_REPO")
        or env.get("MN_MEMBRANE_REPO")
        or DEFAULT_MEMBRANE_REPO
    ).strip()
...
def _ensure_context_engine_source(env: dict[str, str]) -> Path:
    ...
    if not _valid_context_engine_source(target):
        raise RuntimeError(
            f"Membrane source at {target} is missing Dockerfile after checkout."
        )
...
def _remove_dockerfile_frontend_directive(dockerfile: Path) -> None:
    ...
    first = lines[0].strip()
    if not (
        first.startswith("# syntax=docker/dockerfile:")
        or first.startswith("# syntax = docker/dockerfile:")
    ):
        return
...
def _docker_model_command_available() -> bool:
    ...
    return any(
        line.strip().split()[0].rstrip("*") == "model"
        for line in output.splitlines()
        if line.strip()
    )
...
def _ensure_docker_model_runner() -> None:
    ...
    if _docker_command_ok(["docker", "model", "status"]):
        return
    if _docker_command_ok(
        ["docker", "desktop", "enable", "model-runner"]
    ) and _docker_command_ok(["docker", "model", "status"]):
        return
...
def _runtime_blueprint_env_updates(env: dict[str, str]) -> dict[str, str]:
    ...
    blueprint_source = (
        str(
            env.get("MN_BLUEPRINT_SOURCE")
            or os.getenv("MN_BLUEPRINT_SOURCE")
            or "github"
        )
        .strip()
        .lower()
    ...
    blueprint_repo = normalize_blueprint_repo_value(
        str(
            env.get("MN_BLUEPRINT_REPO")
            or os.getenv("MN_BLUEPRINT_REPO")
            or DEFAULT_BLUEPRINT_REPO
        ).strip()
    )
    blueprint_local = str(
        env.get("MN_BLUEPRINT_LOCAL") or os.getenv("MN_BLUEPRINT_LOCAL") or ""
    ).strip()
    host_home_dir = str(
        env.get("MN_HOST_HOME_DIR") or os.getenv("MN_HOST_HOME_DIR") or DIR
    ).strip()
    configured_runs_root = str(
        env.get("MN_RUNS_ROOT") or os.getenv("MN_RUNS_ROOT") or ""
    ).strip()
    host_artifacts_dir = str(
        env.get("MN_HOST_ARTIFACTS_DIR")
        or os.getenv("MN_HOST_ARTIFACTS_DIR")
        or configured_runs_root
    ).strip()
    ...
    container_runs_root = str(
        env.get("MN_CONTAINER_RUNS_ROOT")
        or os.getenv("MN_CONTAINER_RUNS_ROOT")
        or DEFAULT_CONTAINER_RUNS_ROOT
    ).strip()
    ...
    updates: dict[str, str] = {
        ...
        "MN_BLUEPRINT_WEB_UI_PUBLIC_HOST": str(
            env.get("MN_BLUEPRINT_WEB_UI_PUBLIC_HOST")
            or DEFAULT_BLUEPRINT_WEB_UI_PUBLIC_HOST
        ).strip(),
        ...
        "MN_AUTO_PORT_START": str(
            env.get("MN_AUTO_PORT_START")
            or os.getenv("MN_AUTO_PORT_START")
            or DEFAULT_AUTO_PORT_START
        ).strip(),
        "MN_AUTO_PORT_END": str(
            env.get("MN_AUTO_PORT_END")
            or os.getenv("MN_AUTO_PORT_END")
            or DEFAULT_AUTO_PORT_END
        ).strip(),
...
def _ensure_host_artifacts_dir(env: dict[str, str]) -> None:
    for path_text in (
        ...
        try:
            ...
            logger.warning(
                "Failed to create shared artifact directory %s",
                path_text,
                exc_info=True,
            )


def _runtime_endpoint_snapshot(
    env: dict[str, str], web_ui_available: bool = False
) -> dict[str, object]:
    api_host = _native_endpoint_host(str(env.get("MN_API_HOST") or DEFAULT_HOST))
    api_port = _valid_port_text(
        str(env.get("MN_API_PORT") or DEFAULT_API_PORT), DEFAULT_API_PORT
    )
    api_base_url = str(
        env.get("MN_API_BASE_URL") or f"http://{api_host}:{api_port}/api/v1"
    )
    ...
    grpc_port = _valid_port_text(
        str(
            env.get("MN_GRPC_ADVERTISE_PORT")
            or env.get("MN_GRPC_PORT")
            or DEFAULT_GRPC_PORT
        ),
        DEFAULT_GRPC_PORT,
    )
    ...
    if web_ui_available:
        web_ui_host = _native_endpoint_host(
            str(env.get("MN_WEB_UI_HOST") or DEFAULT_HOST)
        )
        web_ui_port = _valid_port_text(
            str(env.get("MN_WEB_UI_PORT") or DEFAULT_WEB_UI_PORT), DEFAULT_WEB_UI_PORT
        )
        snapshot["web_ui"] = {
    ...
    native_sdk_host = _native_endpoint_host(
        str(env.get("MN_NATIVE_SDK_GRPC_HOST") or DEFAULT_NATIVE_SDK_GRPC_HOST)
    )
    ...
    snapshot["native_sdk_grpc"] = {
        "target": str(
            env.get("MN_NATIVE_SDK_GRPC_TARGET")
            or f"{DEFAULT_NATIVE_SDK_GRPC_TARGET_HOST}:{native_sdk_port}"
        ),
...
def _read_runtime_api_health(
    api_host: str, api_port: str, *, timeout_seconds: float = 2.0
...
def _same_blueprint_location(active: str, expected: str) -> bool:
    ...
    return bool(
        ...
        and active_normalized == expected_normalized
    )
...
def _runtime_api_config_mismatches(
    env: dict[str, str], health: Optional[dict[str, Any]]
    ...
    if (
        ...
        and not _same_blueprint_location(active_blueprint, expected_blueprint)
    ):
        mismatches.append(("blueprint repo", active_blueprint, expected_blueprint))

    expected_runs_root = str(
        env.get("MN_RUNS_ROOT") or env.get("MN_HOST_ARTIFACTS_DIR") or ""
    ).strip()
    active_runs_root = str(
        health.get("runs_root") or health.get("runsRoot") or ""
    ).strip()
    if (
        active_runs_root
        and expected_runs_root
        and not _same_runtime_path(active_runs_root, expected_runs_root)
    ):
        mismatches.append(("runs root", active_runs_root, expected_runs_root))
    return mismatches


def _write_runtime_endpoints_file(
    env: dict[str, str], web_ui_available: bool = False
) -> dict[str, object]:
    ...
    RUNTIME_ENDPOINTS_FILE.write_text(
    ...
    try:
        ...
        logger.debug(
            "Failed to chmod runtime endpoint file %s",
...
def _ensure_compose_native_port_settings(env: dict[str, str]) -> dict[str, str]:
    ...
    if not explicit_openshell_endpoint:
        ...
        if persisted_openshell_host in {
            "127.0.0.1",
            "localhost",
            "::1",
        } and published_openshell_host not in {"127.0.0.1", "localhost", "::1"}:
    ...
    native_sdk_proxy_target_host = (
        adjusted.get("MN_NATIVE_SDK_GRPC_PROXY_TARGET_HOST")
        or DEFAULT_NATIVE_SDK_GRPC_TARGET_HOST
    )
    ...
    if existing_native_sdk_target:
        ...
        native_sdk_target = (
            f"{DEFAULT_NATIVE_SDK_GRPC_COMPOSE_SERVICE}:{native_sdk_proxy_port}"
        )
    ...
    updates = {
        ...
        "MN_DOCKER_MODEL_RUNNER_PROXY_ENABLED": adjusted.get(
            "MN_DOCKER_MODEL_RUNNER_PROXY_ENABLED"
        )
        or "true",
        "MN_DOCKER_MODEL_RUNNER_PROXY_BIND_HOST": adjusted.get(
            "MN_DOCKER_MODEL_RUNNER_PROXY_BIND_HOST"
        )
        or "0.0.0.0",
        "MN_DOCKER_MODEL_RUNNER_PROXY_PORT": _valid_port_text(
            str(
                adjusted.get("MN_DOCKER_MODEL_RUNNER_PROXY_PORT")
                or DEFAULT_MODEL_RUNNER_PROXY_PORT
            ),
            DEFAULT_MODEL_RUNNER_PROXY_PORT,
        ),
        "MN_DOCKER_MODEL_RUNNER_PROXY_TARGET_HOST": adjusted.get(
            "MN_DOCKER_MODEL_RUNNER_PROXY_TARGET_HOST"
        )
        ...
        "MN_ARTIFACT_PUBLISH_HOST": adjusted.get("MN_ARTIFACT_PUBLISH_HOST")
        or "127.0.0.1",
...
def _ensure_compose_cluster_bind_settings(
    env: dict[str, str], advertised_host: str
) -> dict[str, str]:
    adjusted = dict(env)
    publish_host = _network_publish_host(advertised_host)
...
def _ensure_compose_cluster_port_settings(
    ...
    *,
    ...
    selected_redis_port = _resolve_published_redis_port(
        ...
        else (env_redis_port or adjusted.get("MN_REDIS_PORT")),
    ...
    adjusted.update(updates)
    adjusted = _ensure_redis_ha_settings(
        adjusted, advertised_host=advertised_host, cluster=True
    )
    if adjusted.get("MN_REDIS_HA_MODE") == "sentinel":
        updates.update(
            {key: adjusted[key] for key in REDIS_HA_ENV_KEYS if key in adjusted}
        )
    _write_env_file_values(RUNTIME_COMPOSE_ENV, updates)
...
def _ensure_compose_internal_redis_settings(
    ...
    *,
    ...
    redis_password = _resolve_redis_password(adjusted)
    redis_host = (
        network_redis_host or os.getenv("MN_NETWORK_REDIS_HOST", "").strip() or "redis"
    )
    redis_port = network_redis_port or REDIS_CONTAINER_PORT

    adjusted["MN_REDIS_PASSWORD"] = redis_password
    adjusted["MN_REDIS_URL"] = (
        f"redis://:{redis_password}@redis:{REDIS_CONTAINER_PORT}/0"
    )
    adjusted["MN_CONTEXT_REDIS_URL"] = (
        f"redis://:{redis_password}@redis:{REDIS_CONTAINER_PORT}/1"
    )
    ...
    adjusted = _ensure_redis_ha_settings(
        adjusted,
        advertised_host=str(adjusted.get("MN_NETWORK_ADVERTISE_HOST") or redis_host),
        cluster=True,
    )
...
def _resolve_network_seed_redis_port(host: str, requested_port: Optional[int]) -> int:
    ...
    configured_port: object = (
        requested_port if requested_port is not None else (env_port or persisted)
    )
    publish_host = _network_publish_host(host)
    ...
    _write_env_file_values(
        NETWORK_REDIS_ENV_FILE, {"MN_REDIS_PORT": str(selected_port)}
    )
    return selected_port


def _validate_remote_redis_details(
    handshake: dict, seed_host: str, token: str
) -> tuple[str, int, str]:
    ...
    if not redis_host or redis_port is None or not redis_url:
        print_error(
            console,
            f"Remote node at {seed_host} did not advertise complete Redis details.",
        )
        raise typer.Exit(1)
    ...
    if (
        ...
        print_error(
            console, f"Remote node at {seed_host} returned an invalid Redis URL."
        )
        raise typer.Exit(1)
    if not parsed.password:
        print_error(
            console,
            f"Remote node at {seed_host} did not advertise password-authenticated Redis.",
        )
        raise typer.Exit(1)
...
def _primary_redis_details(fallback_token: str) -> Optional[tuple[str, int, str]]:
    ...
    password = str(env.get("MN_REDIS_PASSWORD") or "").strip() or (
        _redis_password_from_url(redis_url) if redis_url else ""
    )
    if not password:
...
def _configure_worker_redis_replica(
    ...
    if primary is None:
        print_warning(
            console,
            "Could not determine primary Redis details; worker Redis replication was skipped.",
        )
        return None

    primary_host, primary_port, primary_password = primary
    worker_redis_host, worker_redis_port, worker_redis_url = (
    ...
    if worker_redis_host == primary_host and int(worker_redis_port) == int(
        primary_port
    ):
        print_warning(
            console,
            "Worker Redis replication was skipped because the worker advertised the primary Redis endpoint.",
        )
        return None
    ...
    try:
        ...
        try:
            _redis_command(
                primary_host, primary_port, primary_password, "WAIT", "1", "1000"
            )
        except Exception:
            logger.debug(
                "Primary Redis WAIT after worker replica setup failed", exc_info=True
            )
    except Exception as exc:
        from mn_sdk.errors import AppError
...
def _configure_local_compose_redis_replica(env: dict[str, str]) -> str | None:
    ...
    if (
        not primary_host
        or primary_port is None
        or not primary_password
        or not local_password
    ):
        from mn_sdk.errors import AppError
    ...
    if primary_host in {"localhost", "127.0.0.1"} and int(primary_port) == int(
        local_port
    ):
        return None
...
def runtime_compose_cmd(*args: str) -> list[str]:
    ...
    model_runner_proxy_override = (
        RUNTIME_COMPOSE_FILE.parent / RUNTIME_MODEL_RUNNER_PROXY_OVERRIDE_FILE
    )
...
def stop_matching_sidecar_processes(command_name: str, display_name: str) -> bool:
    ...
    for line in output.splitlines():
        ...
        seen.add(pid)
        console.print(
            f"   Stopping orphaned {display_name} sidecar process (PID: {pid})..."
        )
        kill_tree(pid)
        stopped = True
    if stopped:
...
def _start_native_sdk_grpc_if_installed(
    ...
    *,
    ...
    if _native_sdk_grpc_command() is None:
        print_warning(
            console,
            "mn-native-sdk-grpc was not found; native model preparation forwarding is unavailable.",
        )
        return False
    ...
    if watchdog_status == 0:
        if (
            _wait_for_tcp(native_host, native_port, timeout_seconds=2.0)
            and not restart_running
        ):
            print_warning(
                console, "Native SDK gRPC watchdog is already running; skipping."
            )
            return True
        if restart_running:
            ...
            print_warning(
                console,
                f"Native SDK gRPC watchdog is already running; restarting it{detail}.",
            )
        else:
            print_warning(
                console,
                "Native SDK gRPC watchdog is running, but the service is not responding; restarting it.",
            )
        try:
    ...
    if child_status == 0:
        try:
            ...
            print_info(
                console,
                f"Restarting existing Native SDK gRPC service (PID {pid}) under watchdog…",
            )
            kill_tree(pid)
            time.sleep(1)
        except (ValueError, OSError):
    ...
    print_info(
        console,
        f"Starting Native SDK gRPC watchdog (model prep on {native_host}:{native_port})…",
    )
    try:
    ...
    if _wait_for_tcp(native_host, native_port, timeout_seconds=10.0):
        print_info(
            console, f"Native SDK gRPC watchdog is running (PID {p_watchdog.pid})."
        )
    else:
        print_warning(
            console,
...
def _wait_for_api(
    api_host: str, api_port: str, *, timeout_seconds: float = 10.0
) -> bool:
    url = _api_http_url(api_host, api_port, "/api/v1/health")
    deadline = time.monotonic() + max(timeout_seconds, 0.0)
    last_error: Optional[Exception] = None
            ...
            with urllib.request.urlopen(url, timeout=0.75) as response:
                ...
                if int(status) < 500:
                    ...
                    if (
                        ...
                        and payload.get("api_contract") == "mirrorneuron.rest.v1"
                    ):
                        return True
        except Exception as exc:
            last_error = exc
...
def _start_api_if_installed(
    ...
    *,
    ...
    if watchdog_status == 0:
        if (
            _wait_for_api(api_host, api_port, timeout_seconds=5.0)
            and not restart_running
        ):
        ...
        if restart_running:
            detail = f" ({restart_reason})" if restart_reason else ""
            print_warning(
                console, f"REST API watchdog is already running; restarting it{detail}."
            )
        else:
            print_warning(
                console,
                "REST API watchdog is running, but the API is not responding; restarting it.",
            )
        try:
    ...
    if child_status == 0:
        try:
            ...
            print_info(
                console, f"Restarting existing REST API (PID {pid}) under watchdog…"
            )
            kill_tree(pid)
            time.sleep(1)
        except (ValueError, OSError):
    ...
    if _wait_for_api(api_host, api_port, timeout_seconds=1.0):
        print_warning(
            console,
            "REST API is responding without the current watchdog; restarting it under watchdog.",
        )
...
def _start_web_ui_watchdog(
    web_ui_dir: Path, env: dict[str, str], web_ui_host: str, web_ui_port: str
) -> subprocess.Popen:
    config = {
...
def _wait_for_web_ui(
    web_ui_host: str, web_ui_port: str, *, timeout_seconds: float = 10.0
) -> bool:
...
def _host_port_from_target(
    target: str, default_host: str, default_port: str
) -> tuple[str, str]:
    if "://" in target:
...
def _native_service_endpoints(
    ip: Optional[str] = None, web_ui_available: bool = False
) -> list[dict[str, str]]:
    runtime_env = _runtime_base_env(runtime_compose_available())
    ...
    if runtime_compose_available():
        grpc_host = _cluster_endpoint_host(
            runtime_env, runtime_env.get("MN_GRPC_BIND_HOST", "127.0.0.1")
        )
        ...
        api_port = runtime_env.get("MN_API_PORT", DEFAULT_API_PORT)
        docker_mode = _docker_network_mode(
            runtime_env.get("MN_DOCKER_NETWORK_MODE"), default="disabled"
        )
        include_internal_cluster_ports = not _docker_network_uses_internal_identity(
            docker_mode
        )
        redis_host, redis_port = _redis_host_port(ip)
        redis_host = _cluster_endpoint_host(runtime_env, redis_host)
        epmd_host = _cluster_endpoint_host(
            runtime_env, runtime_env.get("MN_EPMD_HOST") or _epmd_host()
        )
        epmd_port = runtime_env.get("MN_EPMD_PORT", DEFAULT_EPMD_PORT)
        dist_host = _cluster_endpoint_host(
            runtime_env, runtime_env.get("MN_DIST_HOST") or _dist_host()
        )
...
def _print_service_endpoints(ip: Optional[str], web_ui_available: bool):
    ...
    if _docker_network_uses_internal_identity(docker_mode):
        console.print(
            "[dim]Redis and Erlang cluster traffic use Docker internal networking.[/dim]"
        )
...
def _start_web_ui_if_installed(runtime_env: Optional[dict[str, str]] = None) -> bool:
    if compose_web_ui_enabled():
        ...
        web_ui_port = _valid_port_text(
            str(
                env.get("MN_WEB_UI_PORT")
                or os.getenv("MN_WEB_UI_PORT", DEFAULT_WEB_UI_PORT)
            ),
            DEFAULT_WEB_UI_PORT,
        )
        try:
    ...
    if (
        ...
        print_warning(
            console,
            "Web UI is responding without the current watchdog; restarting it under watchdog.",
        )
        stopped_untracked = stop_matching_sidecar_processes(
            "mn-web-ui-server", "Web UI"
        )
    if watchdog_status == 0:
        ...
        try:
            ...
            print_warning(
                console,
                "Web UI watchdog is running, but the page is not responding; restarting it.",
            )
            kill_tree(watchdog_pid)
            time.sleep(1)
        except (ValueError, OSError):
    ...
    if child_status == 0:
        try:
            ...
            print_info(
                console, f"Restarting existing Web UI (PID {pid}) under watchdog…"
            )
            kill_tree(pid)
            time.sleep(1)
        except (ValueError, OSError):
    ...
    print_info(
        console,
        f"Starting mn-web-ui watchdog (static server on {web_ui_host}:{web_ui_port})…",
    )
    try:
        p_web_watchdog = _start_web_ui_watchdog(
            web_ui_dir, env, web_ui_host, web_ui_port
        )
    except FileNotFoundError:
...
def _build_core_docker_run_command(
    ...
    *,
    ...
    cmd.extend(
        [
            "-e",
            f"MN_GRPC_AUTH_TOKEN={env.get('MN_GRPC_AUTH_TOKEN', FIXED_GRPC_AUTH_TOKEN)}",
        ]
    )
    cmd.extend(
        [
            "-e",
            f"{GRPC_ADMIN_TOKEN_ENV}={env.get(GRPC_ADMIN_TOKEN_ENV, FIXED_GRPC_ADMIN_TOKEN)}",
        ]
    )
    ...
    if env.get("MN_MODEL_SERVICE_NODE_NAME"):
        cmd.extend(
            ["-e", f"MN_MODEL_SERVICE_NODE_NAME={env['MN_MODEL_SERVICE_NODE_NAME']}"]
        )
    if env.get("MN_NATIVE_SDK_GRPC_TARGET"):
        cmd.extend(
            ["-e", f"MN_NATIVE_SDK_GRPC_TARGET={env['MN_NATIVE_SDK_GRPC_TARGET']}"]
        )
    ...
    if env.get("MN_NATIVE_SDK_GRPC_ADVERTISE_HOST"):
        cmd.extend(
            [
                "-e",
                f"MN_NATIVE_SDK_GRPC_ADVERTISE_HOST={env['MN_NATIVE_SDK_GRPC_ADVERTISE_HOST']}",
            ]
        )
    if env.get("MN_NATIVE_SDK_GRPC_ADVERTISE_PORT"):
        cmd.extend(
            [
                "-e",
                f"MN_NATIVE_SDK_GRPC_ADVERTISE_PORT={env['MN_NATIVE_SDK_GRPC_ADVERTISE_PORT']}",
            ]
        )
    ...
    cmd.extend(
        [
            "-e",
            f"MN_GRPC_ADVERTISE_PORT={env.get('MN_GRPC_ADVERTISE_PORT', env['MN_GRPC_PORT'])}",
        ]
    )
    ...
    cmd.extend(
        [
            "-e",
            f"MN_RUNS_ROOT={env.get('MN_CONTAINER_RUNS_ROOT', DEFAULT_CONTAINER_RUNS_ROOT)}",
        ]
    )
    cmd.extend(
        [
            "-e",
            f"MN_SHARED_STORAGE_ROOT={env.get('MN_RUNTIME_SHARED_STORAGE_ROOT', DEFAULT_RUNTIME_SHARED_STORAGE_ROOT)}",
        ]
    )
    cmd.extend(
        [
            "-e",
            f"MN_RUNTIME_SHARED_STORAGE_ROOT={env.get('MN_RUNTIME_SHARED_STORAGE_ROOT', DEFAULT_RUNTIME_SHARED_STORAGE_ROOT)}",
        ]
    )
    cmd.extend(
        [
            "-e",
            f"MN_AUTO_PORT_START={env.get('MN_AUTO_PORT_START', DEFAULT_AUTO_PORT_START)}",
        ]
    )
    cmd.extend(
        ["-e", f"MN_AUTO_PORT_END={env.get('MN_AUTO_PORT_END', DEFAULT_AUTO_PORT_END)}"]
    )
    ...
    if requested_docker_mode != "disabled" and node_alias:
        cmd.extend(
            _docker_network_run_args(requested_docker_mode, network_name, node_alias)
        )
    ...
    if system_name == "Darwin":
        cmd.extend(
            ["-p", f"{core_publish_host}:{env['MN_GRPC_PORT']}:{env['MN_GRPC_PORT']}"]
        )
        cmd.extend(
            [
                "-p",
        ...
        if requested_docker_mode == "disabled":
            ...
            cmd.extend(
                [
                    "-p",
                    f"{dist_publish_host}:{env['MN_DIST_PORT']}:{env['MN_DIST_PORT']}",
                ]
            )
        cmd.extend(
            [
                "-e",
                f"MN_REDIS_URL={env.get('MN_REDIS_URL', 'redis://host.docker.internal:6379/0')}",
            ]
        )
    ...
    if (openshell_config_dir / "gateways" / "openshell").is_dir():
        ...
        cmd.extend(
            ["-v", f"{openshell_config_dir}:/opt/mirror_neuron/.config/openshell:ro"]
        )

    host_home_dir = str(env.get("MN_HOST_HOME_DIR") or DIR)
    host_artifacts_dir = str(
        env.get("MN_HOST_ARTIFACTS_DIR") or Path(host_home_dir).expanduser() / "runs"
    )
    container_runs_root = str(
        env.get("MN_CONTAINER_RUNS_ROOT") or DEFAULT_CONTAINER_RUNS_ROOT
    )
    host_blob_store_dir = str(
        env.get("MN_HOST_BLOB_STORE_DIR") or Path(host_home_dir).expanduser() / "blobs"
    )
    container_blob_store_root = str(
        env.get("MN_CONTAINER_BLOB_STORE_ROOT") or DEFAULT_CONTAINER_BLOB_STORE_ROOT
    )
    host_shared_storage_root = str(
        env.get("MN_HOST_SHARED_STORAGE_ROOT")
        or env.get("MN_SHARED_STORAGE_ROOT")
        or Path(host_home_dir).expanduser() / "shared"
    )
    runtime_shared_storage_root = str(
        env.get("MN_RUNTIME_SHARED_STORAGE_ROOT") or DEFAULT_RUNTIME_SHARED_STORAGE_ROOT
    )
    runtime_bundle_cache_dir = str(
        env.get("MN_BUNDLE_CACHE_DIR")
        or f"{runtime_shared_storage_root.rstrip('/')}/bundle_cache"
...
def _start_server(
    ...
    *,
    ...
    if check_status(API_PID_FILE) == 0:
        ...
        if compose_runtime:
            ...
            if not core_running:
                print_info(
                    console,
                    "MirrorNeuron Core is not running; starting Docker runtime (Compose)…",
                )
            elif _running_core_has_stale_grpc_tokens():
                print_info(
                    console,
                    "MirrorNeuron Core has stale gRPC tokens; recreating Docker runtime (Compose)…",
                )
            ...
            try:
                ...
                if force_runtime_recreate:
                    compose_args.extend(
                        ["--force-recreate", "redis", "mirror-neuron-core"]
                    )
                _ensure_litellm_gateway_host_config()
                subprocess.run(
                    runtime_compose_cmd(*compose_args),
                    check=True,
                    stdout=subprocess.DEVNULL,
                    env=env,
                )
                if not core_running:
                    print_info(
                        console,
                        "Docker runtime is running (Compose project: mirror-neuron).",
                    )
            except (FileNotFoundError, subprocess.CalledProcessError):
        ...
        if api_mismatches:
            ...
            print_warning(
                console,
                f"REST API runtime config changed; restarting API ({mismatch_details}).",
            )
        ...
        endpoint_snapshot = _write_runtime_endpoints_file(
            env, web_ui_available=web_ui_available
        )
        print_info(console, f"Runtime endpoints: {RUNTIME_ENDPOINTS_FILE}")
        logger.info(
            "Refreshed MirrorNeuron runtime endpoints: %s",
            endpoint_snapshot.get("api", {}),
        )
    ...
    if not compose_runtime:
        try:
            docker_status = subprocess.run(
                ["docker", "inspect", "-f", "{{.State.Running}}", "mirror-neuron-core"],
                capture_output=True,
                text=True,
            )
    ...
    if ip and not token:
        print_error(
            console,
            "Federation registration requires the token printed by 'mn runtime start'.",
        )
        raise typer.Exit(1)
    ...
    network_name = _docker_network_name(docker_network_name)
    use_internal_identity = _docker_network_uses_internal_identity(
        requested_docker_mode
    )
    ...
    env = _ensure_runtime_api_token(env, persist_compose=compose_runtime)
    env = _ensure_runtime_grpc_tokens(env, persist_compose=compose_runtime)
    reconnecting_joined_node = bool(
        compose_runtime and not ip and _persisted_join_profile(env)
    )
    if compose_runtime:
        env = _ensure_compose_native_port_settings(env)
    ...
    if join_handshake:
        ...
        seed_redis_port = redis_port or _parse_port(
            join_handshake["redis_port"], REDIS_CONTAINER_PORT
        )
        ...
        if use_internal_identity and node_alias:
            ...
            seed_redis_port = redis_port or (
                _parse_port(
                    env.get("MN_NETWORK_REDIS_PORT")
                    or env.get("MN_REDIS_PORT")
                    or os.getenv("MN_REDIS_PORT"),
                    REDIS_CONTAINER_PORT,
                )
            )
    ...
    env.setdefault(
        "MN_BLUEPRINT_WEB_UI_PUBLIC_HOST", DEFAULT_BLUEPRINT_WEB_UI_PUBLIC_HOST
    )
    env.setdefault(
        "MN_BLUEPRINT_WEB_UI_PORT_START", DEFAULT_BLUEPRINT_WEB_UI_PORT_START
    )
    ...
    env.setdefault(
        "MN_GRPC_TARGET", f"localhost:{env.get('MN_GRPC_PORT', DEFAULT_GRPC_PORT)}"
    )
    ...
    env.setdefault(
        "MN_NATIVE_SDK_GRPC_TARGET",
        f"{DEFAULT_NATIVE_SDK_GRPC_TARGET_HOST}:{env['MN_NATIVE_SDK_GRPC_PORT']}",
    )
    ...
    if requested_docker_mode != "disabled" and node_alias:
        env.update(_docker_network_env(requested_docker_mode, network_name, node_alias))
    if _generated_node_setting_should_update(
        "MN_NODE_NAME", env.get("MN_NODE_NAME"), local_node_name
    ):
    ...
    if join_handshake:
        ...
        if compose_runtime:
            _write_env_file_values(
                RUNTIME_COMPOSE_ENV,
                {
                    ...
                    "MN_NATIVE_SDK_GRPC_TARGET": env.get(
                        "MN_NATIVE_SDK_GRPC_TARGET", ""
                    ),
                    "MN_NATIVE_SDK_GRPC_PROXY_PORT": env.get(
                        "MN_NATIVE_SDK_GRPC_PROXY_PORT", ""
                    ),
                    "MN_NATIVE_SDK_GRPC_PROXY_TARGET_HOST": env.get(
                        "MN_NATIVE_SDK_GRPC_PROXY_TARGET_HOST", ""
                    ),
                    "MN_NATIVE_SDK_GRPC_PROXY_TARGET_PORT": env.get(
                        "MN_NATIVE_SDK_GRPC_PROXY_TARGET_PORT", ""
                    ),
                    ...
                    **{
                        key: env[key]
                        for key in NODE_ADVERTISEMENT_ENV_KEYS
                        if key in env
                    },
                    **{key: env[key] for key in SHARED_STORAGE_ENV_KEYS if key in env},
                    **{key: env[key] for key in SYNCTHING_ENV_KEYS if key in env},
                    ...
                    **(
                        _docker_network_env(
                            requested_docker_mode, network_name, node_alias
                        )
                        if requested_docker_mode != "disabled" and node_alias
                        else {}
                    ),
        ...
        if not reconnecting_joined_node and _generated_cluster_setting_should_update(
            env.get("MN_CLUSTER_NODES"), local_node_name
        ):
            env["MN_CLUSTER_NODES"] = local_node_name
        if (
            compose_runtime
            and not reconnecting_joined_node
            and _compose_supports_redis_password()
        ):
            redis_password = _resolve_redis_password(env)
        ...
        _write_env_file_values(
            RUNTIME_COMPOSE_ENV,
            {
                ...
                **{key: env[key] for key in NODE_ADVERTISEMENT_ENV_KEYS if key in env},
                ...
                "MN_DOCKER_NETWORK_MODE": env.get("MN_DOCKER_NETWORK_MODE", "disabled"),
                "MN_DOCKER_NETWORK_NAME": env.get(
    ...
    if compose_runtime:
        ...
        try:
            ...
            print_info(
                console, "Docker runtime is running (Compose project: mirror-neuron)."
            )
        except (FileNotFoundError, subprocess.CalledProcessError):
        ...
        subprocess.run(
            ["docker", "rm", "-f", "mirror-neuron-core"],
            stderr=subprocess.DEVNULL,
            stdout=subprocess.DEVNULL,
        )
    ...
    if api_started:
        ...
        print_info(console, f"Runtime endpoints: {RUNTIME_ENDPOINTS_FILE}")
        logger.info(
            "Wrote MirrorNeuron runtime endpoints: %s", endpoint_snapshot.get("api", {})
        )
...
def _prepare_running_compose_exposure(
    ...
    *,
    ...
    if _generated_node_setting_should_update(
        ...
        env["MN_MODEL_SERVICE_NODE_NAME"] = env["MN_NODE_NAME"]
    if _generated_cluster_setting_should_update(
        env.get("MN_CLUSTER_NODES"), local_node_name
    ):
        env["MN_CLUSTER_NODES"] = local_node_name
    env["MN_NODE_ROLE"] = env.get("MN_NODE_ROLE") or "runtime"
